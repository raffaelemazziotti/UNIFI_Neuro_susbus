...
These signals exist because [ions](https://en.wikipedia.org/wiki/Ion) [move across cell membranes]{.highlight}.
...
## Example: Calcium Ions (Ca²⁺){#calcium}
...
> Opening Ca²⁺ channels causes an `inward` flux of Ca²⁺ ions.
...
![](images/ions_pump.gif){width="500" fig-align="right"}
...
This reflects $K^+$ dominance at rest.
:::

---

## Voltage Change {#voltage_change}

`If channels open` for an ion, the `membrane potential changes`. 

>If a positive ion moves inside the cell, the membrane potential becomes less negative; if it moves outside, it becomes more negative. For negative ions, the effect is reversed.
...
This process is called [depolarization]{.highlight}.
:::
:::
::: column
::: info-box
...
This process is called [hyperpolarization]{.highlight}.
...
## Graded Potentials {#graded_potential}
...
::: info-box
[Integration]{.highlight} mainly occurs at:
...
>**Action potential**: A `large`, `rapid`, and [stereotyped]{.highlight} change in membrane potential, originated at the level of the [axon hillock](1_intro.html#hillock), that propagates along the axon.
::: 
::: column
...
- Do not move ions against gradients (same as above)
...
Voltage is dominated by `voltage-gated dynamics`. After the action potential:
